...
MM_ConcurrentGC::initialize(MM_EnvironmentBase *env)
{
	...
	if (_extensions->configuration->isSnapshotAtTheBeginningBarrierEnabled()) {
		...
		(*mmPrivateHooks)->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_CLEARED, tlhCleared, OMR_GET_CALLSITE(), (void *)_markingScheme);
	}
...
MM_ConcurrentGC::tlhRefreshed(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
	...
	omrtty_printf("[tlhRefreshed] [%i] TLH [%p] -> [%p] [%i] \n", env->getWorkerID(), event->cacheBase, event->cacheTop, ((uintptr_t)event->cacheTop - (uintptr_t)event->cacheBase) / 1000 );

	if (env->getExtensions()->isSATBBarrierActive(env)) {

		Assert_MM_false(((MM_ParallelGlobalGC *)env->getExtensions()->getGlobalCollector())->getMarkingScheme()->isMarked((omrobjectptr_t) event->cacheBase));
		Assert_MM_false(((MM_ParallelGlobalGC *)env->getExtensions()->getGlobalCollector())->getMarkingScheme()->isMarked((omrobjectptr_t) ((uintptr_t)event->cacheTop - 1)));

		if (env->getExtensions()->debugSATBlevel >= 1) {

			omrtty_printf("\t [MM_ConcurrentGC] [SATB] [tlhRefreshed] [%i] Premark TLH [%p] -> [%p] [%i] \n", env->getWorkerID(), event->cacheBase, event->cacheTop, ((uintptr_t)event->cacheTop - (uintptr_t)event->cacheBase) / 1000 );
		}
		((MM_MarkingScheme *)userData)->preMarkTLH(env, (MM_MemorySubSpace *)event->subSpace, event->cacheBase, event->cacheTop);

		Assert_MM_true(((MM_ParallelGlobalGC *)env->getExtensions()->getGlobalCollector())->getMarkingScheme()->isMarked((omrobjectptr_t) event->cacheBase));
		Assert_MM_true(((MM_ParallelGlobalGC *)env->getExtensions()->getGlobalCollector())->getMarkingScheme()->isMarked((omrobjectptr_t) ((uintptr_t)event->cacheTop - 1)));
		Assert_MM_false(((MM_ParallelGlobalGC *)env->getExtensions()->getGlobalCollector())->getMarkingScheme()->isMarked((omrobjectptr_t) ((uintptr_t)event->cacheTop)));
	}
}

void
MM_ConcurrentGC::tlhCleared(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
//	if (event->cacheAlloc == event->cacheTop)
//		return;


	//if (env->getExtensions()->isSATBBarrierActive(env) ) {
		if (env->getExtensions()->debugSATBlevel >= 1 && event->cacheAlloc != event->cacheTop) {
			OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
			omrtty_printf("[MM_ConcurrentGC] [SATB] [tlhCleared] [%i] CLEAR TLH [%p]-> [%p] [%i]  \n", env->getWorkerID(), event->cacheAlloc, event->cacheTop, ((uintptr_t)event->cacheTop - (uintptr_t)event->cacheAlloc) / 1000);
		}
		((MM_MarkingScheme *)userData)->clearPremarkedTLH(env, (MM_MemorySubSpace *)event->subSpace, event->cacheAlloc, event->cacheTop);
	//}
}
...
MM_ConcurrentGC::timeToKickoffConcurrent(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription)
{
	...
	if (_extensions->configuration->isSnapshotAtTheBeginningBarrierEnabled()) {
		threshold = (5.7 * 1000000) ;// 7 * 100000000; // //9 * 100000000; //_stats.getKickoffThreshold() * 1.35;
	}
