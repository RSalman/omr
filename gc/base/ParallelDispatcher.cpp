...
MM_ParallelDispatcher::recomputeActiveThreadCountForTask(MM_EnvironmentBase *env, MM_Task *task, uintptr_t threadCount)
{
	...
	/* Account for Adaptive Threading. Ignore if
	 *
	 *  1) User forced a thread count (e.g Xgcthreads)
	 *  2) Adaptive threading flag is not set (-XX:-AdaptiveGCThreading)
	 *  3) The task wasn't recommended a thread count (currently only recommended for STW Scavenge Tasks)
	 */
	if (_extensions->adaptiveThreadigEnabled() && (task->getRecommendedWorkingThreads() != UDATA_MAX)) {
		 ...
		 */
		taskActiveThreadCount = OMR_MIN(_threadCountMaximum, task->getRecommendedWorkingThreads());
